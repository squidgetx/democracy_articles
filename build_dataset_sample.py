...
from lib import wc, get_sentences
...
for year in years:
    ...
    prop = 1 if len(articles) < N else N/len(articles)
    articles_sample = articles.train_test_split(test_size=prop)['train']
...
util.write_tsv(records, 'articles.sample.10k.tsv')
util.write_tsv(sentences, 'sentences.sample.10k.tsv')
